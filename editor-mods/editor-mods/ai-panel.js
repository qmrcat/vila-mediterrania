...
export function bindAi({ $, fillSelect, toast, onMod, renderMod, context }) {
  ...
  let proxyProviders = [];
  let proxyInstalled = true;   // fals si el servidor no porta ai-proxy.mjs
  ...
  function refreshMode() {
    ...
    $('#ai-state').textContent = proxy
      ...
      : !proxyInstalled
        ? 'La passarel·la no està instal·lada: falta ai-proxy.mjs a l’arrel del joc i les dues línies de server.mjs. '
          + 'Mentrestant pots fer servir la clau d’aquest navegador. El 404 de la consola és aquesta comprovació.'
   ...
   *
   * Si la passarel·la no està instal·lada, això deixa un 404 a la consola del
   * navegador: és una comprovació de capacitat, no un error, i el panell
   * continua funcionant amb la clau escrita aquí.
   */
  async function probeProxy() {
    try {
      const response = await fetch('/api/ai/status');
      if (response.status === 404) { proxyInstalled = false; proxyProviders = []; }
      else if (!response.ok) throw new Error();
      else proxyProviders = (await response.json()).providers ?? [];
    } catch {
