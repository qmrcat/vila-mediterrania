...
export const DESIGN_KEY='vila-mediterrania:designs:v2';
...
export function newDesign(){
  return {version:2,id:designId(),name:'El meu edifici',width:1,depth:1,middleCount:1,
...
export function validateDesign(d){
  if(!d||![1,2].includes(d.version)||typeof d.id!=='string'||! /^[a-zA-Z0-9_-]{1,80}$/.test(d.id)||typeof d.name!=='string'||!d.name.trim()||d.name.length>60||!integer(d.width,1,3)||!integer(d.depth,1,3)||!integer(d.middleCount,0,3))throw new Error('El disseny ha de tenir un nom, una mida d’1 a 3 cel·les i de 0 a 3 plantes intermèdies.');
  ...
  return {version:2,id:d.id,name:d.name.trim(),width:d.width,depth:d.depth,middleCount:d.middleCount,
    ground:Array.from(d.ground,c=>cell(c,false)),middle:Array.from(d.middle,c=>cell(c,true)),roof:Array.from(d.roof,c=>{
      if(!c||!['tile','shed','flat'].includes(c.type)||!hex(c.color)||!integer(c.direction,0,3))throw new Error('Hi ha una coberta no vàlida.');
...
export function validateCatalog(data){
  if(!data||data.format!=='vila-buildings'||![1,2].includes(data.version)||!Array.isArray(data.designs)||data.designs.length>100)throw new Error('Aquest fitxer no és una col·lecció de dissenys compatible (màxim 100).');
...
export const catalogFile=designs=>({format:'vila-buildings',version:2,designs:designs.map(validateDesign)});
export function loadDesigns(storage){const raw=storage.getItem(DESIGN_KEY)??storage.getItem('vila-mediterrania:designs:v1');return raw===null?[]:validateCatalog(JSON.parse(raw));}
...
export const customFloors=(d,index)=>1+(d.middle[index]?d.middleCount:0);
