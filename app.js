...
const STORAGE_KEY='vila-mediterrania:v35';
...
function landmarkOptions(){return {type:tool,size:LANDMARK_TYPES[tool]?.sizes.length===1?LANDMARK_TYPES[tool].sizes[0]:Number($(tool==='playground'?'#playground-size':'#civic-size').value),direction:Number($(MONUMENT_TOOLS.has(tool)?'#monument-direction':'#landmark-direction').value)};}
...
function describeLandmark(){
  ...
  $('#civic-size-options').hidden=!['hospital','school'].includes(tool);
...
async function init(){
  ...
  for(const id of ['playground-size','civic-size','landmark-direction','monument-direction'])$('#'+id).addEventListener('change',describeLandmark);
