...
export function renderCustomBuildings(world,add,UNIT,FLOOR,entranceSteps=createEntranceSteps(world)){
  ...
  for(const b of world.customBuildings??[]){
    ...
    for(let i=0;i<design.ground.length;i++){
      ...
      for(let level=0;level<floors;level++){
        const cell=level===0?design.ground[i]:design.middle[i],bottom=level*FLOOR;
        if(cell.style==='arcade'){
          ...
          for(let d=0;d<4;d++){
            const next=neighbours[d];if(next>=0&&customFloors(design,next)>level&&(level===0?design.ground[next]:design.middle[next]).style==='solid')continue;
      ...
      const roof=design.roof[i],top=base+floors*FLOOR,angle=a+roof.direction*Math.PI/2,c=Math.cos(angle),s=Math.sin(angle);
      ...
      if(roof.type==='flat'){
        ...
        for(let d=0;d<4;d++){
          ...
          face('box',design.ground[i].color,d,0,floors*FLOOR+.20,.605,1.30,.29,.09);
        }
      }else if(roof.type==='shed'){
        rp('shedWall',design.ground[i].color,0,0,0,1.255,1,1.255);
        ...
        rp('gable',design.ground[i].color,0,0,0,1.24,.88,1.24);const slope=Math.atan2(.39,.65);
